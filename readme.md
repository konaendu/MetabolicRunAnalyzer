...
I'm using only 2 x 12min efforts (nasal breathing & 12min all out test) where we measure heart rate and pace and distance.

The Vo2Max is derived from either a) known data or the HIT 12min all out test. (Formula will be updated on relase). 
...
# 2. Turn it into Power. Vo2ss & Vlamax:
...
```java
    ...
    const vlamax = calculateVLAmax(vo2max, bmi, weight, workload);
    function calculateVLAmax(vo2max, bmi, weight, workload) {
        var maderFormula = 0.02049 / (workload * 0.9 / weight) * vo2max * (bmi / 22) * (1 + 0.000025 * age - 0.0000001 * weight);
        return maderFormula;
    }    
```
